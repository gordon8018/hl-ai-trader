...
def main():
    ...
    def fetch_recent_trades(coin: str) -> List[dict]:
        if not MD_TRADES_ENABLED:
            return []
        try:
            data = _exchange.get_recent_trades(coin, limit=100)
    ...
    while True:
        t0 = time.time()
        try:
            # Use adapter to get all mids for UNIVERSE symbols
            mids_raw = _exchange.get_all_mids(list(UNIVERSE))
            allmids = {k: str(v) for k, v in mids_raw.items()}  # Keep string format for compatibility
            now = time.time()
