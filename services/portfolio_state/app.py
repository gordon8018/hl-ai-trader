...
from shared.exchange.base import ExchangeError
...
def _terminal_status(status: str) -> bool:
    """Check if order status is terminal (filled, canceled, or rejected)."""
    s = status.lower()
    return s in ("filled", "canceled", "rejected")
...
def main():
    ...
    while True:
        ...
        if ACCOUNT and (now - last_snapshot_ts) >= POLL_SECONDS:
            ...
            try:
                # Use exchange adapter for account state
                state = _exchange.get_account_state()
                equity = state.equity_usd
                cash = state.cash_usd

                # Get mid prices for mark price
                mids = _exchange.get_all_mids(list(UNIVERSE))

                # Build positions from normalized state
                positions: Dict[str, Position] = {}
                for sym in UNIVERSE:
                    norm_pos = state.positions.get(sym)
                    mark = mids.get(sym, 0.0)
                    if norm_pos and norm_pos.qty != 0:
                        # Convert lowercase side to uppercase for schema
                        side = norm_pos.side.upper()
                        positions[sym] = Position(
                            symbol=sym,
                            qty=norm_pos.qty,
                            entry_px=norm_pos.entry_px,
                            mark_px=mark,
                            unreal_pnl=norm_pos.unreal_pnl,
                            side=side,
                        )
                    else:
                        # FLAT position
                ...
                # Build open orders from normalized state
                open_orders = []
                for o in state.open_orders:
                    ...
                    open_orders.append(
                        OpenOrder(
                            client_order_id=o.client_order_id or o.exchange_order_id or "unknown",
                            exchange_order_id=o.exchange_order_id or None,
                            symbol=o.symbol,
                            side=side,
                            px=o.limit_px,
                            qty=o.qty,
                ...
                for i in range(0, len(orders_to_poll), ORDER_STATUS_BATCH_SIZE):
                    ...
                    for oid_s, meta in batch:
                        ...
                        try:
                            ...
                            if _terminal_status(order_result.status):
                                bus.r.srem(ACTIVE_OIDS_SET, oid_s)
                                bus.r.hdel(OID_CYCLE_MAP, oid_s)
                                bus.r.hdel(OID_META_HASH, oid_s)
                            else:
